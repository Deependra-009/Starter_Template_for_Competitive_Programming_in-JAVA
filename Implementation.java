...
public final class C {
	...
	static int cmp;
	static int fastVar;
	static long zetamod = 999999999999989L;
	...
	public static void main(String[] args) throws Exception {
		...
		for (int tc = 0; tc < t; tc++) {
			...
			for (int qu = 0; qu < m; qu++) {
				...
				Long smaller = ts.floor(b);
				Long greater = ts.ceiling(b);
				
				if (smaller != null && !intersects(a, b, c, smaller)) {
					Yes();
					out.println(smaller);
					continue QU;
				}
				
				if (greater != null && !intersects(a, b, c, greater)) {
					Yes();
					out.println(greater);
					continue QU;
				}
	...
	public static boolean intersects(long a, long b, long c, long m) {
	    // Calculate the discriminant of the quadratic equation
		b -= m;
		long discriminant = (b * b) - (4 * a * c);
		
	    // If the discriminant is negative, the parabola does not intersect the line
	    if (discriminant < 0)
	        return false;
	    return true;
	}
	...
	static class Segment {
		...
		Segment (long ll, long rr, int ii) {
			li = ll;
			ri = rr;
			id = ii;
		}
	...
	static long modDiv(long a, long b){return (a * power(b, gigamod - 2, gigamod) + gigamod) % gigamod;}
 ...
 * int[] arr = new int[] {1810, 1700, 1710, 2320, 2000, 1785, 1780
				, 2130, 2185, 1430, 1460, 1740, 1860, 1100};
